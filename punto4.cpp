...
int main() {
    cv::Mat src = cv::imread("../Data/ave.jpg", cv::IMREAD_GRAYSCALE);
    if (src.empty()) {
    ...
    // Paso 3: Encontrar el contorno MÁS GRANDE (el pájaro)
    int idxMayor = 0;
    double areaMax = 0;
    ...
    // Paso 4: Dibujar SOLO el contorno más grande relleno -> fondo queda negro
    cv::Mat dst = cv::Mat::zeros(src.size(), CV_8UC1);
    cv::drawContours(dst, contornos, idxMayor, cv::Scalar(255), cv::FILLED);

    cv::imshow("Entrada", src);
    cv::imshow("Salida - Sin ruido blanco", dst);
    cv::imwrite("salida4.png", dst);
